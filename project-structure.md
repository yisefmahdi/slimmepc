...
│       │   ├── service-console.blade.php  # PlayStation/Xbox service page (pageKey `console`) — own unique design mirroring step-2/playstation.html; auto-selected by PageController per-pageKey fallback
│       │   └── partials/
...
| 13 | Service-page (CMS) | `/diensten/{slug}` (`service.show`) + `/admin/content/home/section/{...}` (same editor, per service pageKey) | CMS service detail page matching `step-2/reparatie-laptop.html` (Laptop built first). 8 sections: hero, problems, speciality, equipment, example, other, faq, bottom — all driven by `content_blocks` for the given pageKey (`config('cms.service_slugs')` maps slug→pageKey). **Nav auto-populates**: only service pages with seeded `content_blocks` appear in the admin sidebar (filtered in `components/admin/layout.blade.php`) and the public header Diensten dropdown (Laptop ready, the other 8 "Binnenkort") + **PlayStation/Xbox (`console`) now has its own unique design** `service-console.blade.php` (8 sections matching `step-2/playstation.html`). Only `laptopreparatie` and `console-reparatie` are built so far — Laptop uses the shared `landing.service` template; the remaining slugs 404 until content is added |
| 14 | Service-page: PlayStation/Xbox (Console) | `/diensten/console-reparatie` (`service.show`, pageKey `console`) + `/admin/content/console/section/{...}` | Second service page with its **own unique design** mirroring `step-2/playstation.html` (PlayStation blue `#123b6d` gradient, emoji section icons). Built as `resources/views/landing/service-console.blade.php`, auto-selected by the `PageController@service` per-pageKey view fallback (`landing.service-console` exists → used instead of the shared `landing.service`). 8 CMS sections from `$consoleSectionDef` (config/cms.php), all driven by `content_blocks` page=`console`: hero (badge, title1/2/3, description, `problem_list` json, hero image), consoles (title, subtitle, `items` json of 3 cards: PlayStation 5 / Xbox Series X / beide, each name+image), problems, werkwijze (4 steps), motorkap, voorana, services, garantie. **Hero CTA buttons are intentionally NOT editable** — `cta1/cta2` text+url removed from the admin hero schema (fixed via seeder + blade). Mobile hero sizing matches the laptop page fixed-px standard (`text-[30px] sm:text-[52px]`, buttons `px-5 py-2.5 sm:px-7 sm:py-4`). **Fixes applied**: (1) consoles `foreach` reused the global `$c` (home header/footer data used by the footer partial) as its loop var, wiping the footer → renamed loop var to `$ci`; (2) admin `content.js` json-row image preview doubled the path (`/assets/img/landing/` + the already-full `assets/img/landing/...` value) → fixed to `'/' + fv`. Images copied to `public/assets/img/landing/`; `ps5-before/after/inside` source images were missing so `playtios2.png` is used as a placeholder. |
...
| GET | `/diensten/{slug}` | `PageController@service` → `landing.service` (CMS service page + design) — **name `service.show`** | none (guests served from full-page HTML cache `cms.page.html.service.{$pageKey}.{$version}`; 404 when page has no `hero.title1`) |
...
|--------|------|--------------|
| id | bigint (PK, auto) | Primary key |
| page | string | Page key (e.g. `home`, `console`, or any service pageKey) |
| section | string | Section key (`hero`, `why`, `header`, `services`, `shop`, `footer`, `floating`, `process`, and `console` page sections: `hero`, `consoles`, `problems`, `werkwijze`, `motorkap`, `voorana`, `services`, `garantie`) |
