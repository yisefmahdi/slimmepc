...
- **Labels betaalpagina (2026-09-26, op verzoek):** H1 `Betaling klant` → `Betaling formulier`; klantblok zonder `Klantgegevens`-titel, label boven waarde (`Klantnummer/Naam/Adres/Email/Telefoon/Datum`); `Omschrijving` → `Probleem omschrijven`, `Uitgevoerde werkzaamheden` → `Werkzaamheden`, `Starttijd *` → `Van *`, `Eindtijd *` → `Tot *`, `Prijsopbouw` → `Aantal uren`, `Kortingscode` → `Heb je een kortingscode?`, `Voorrijkosten` → `Reiskost` (bedrag via `prTravel`/`Gratis` ongewijzigd).
