...
| PageController | home, service | **home**: landing page with full-page HTML cache (see above). **service($slug)**: CMS service page — loads `Cms::page($pageKey)` where `$pageKey = config('cms.service_slugs')[$slug]` (404 if slug unknown), 404s when `hero.title1` is empty (unbuilt page), same full-page cache pattern as `home` keyed `cms.page.html.service.{$pageKey}.{$version}` (guest-only, auth renders fresh), and injects `Cms::page('home')` for the header/footer |
