...
│       └── ContentBlockSeeder.php     # 63 default blocks (Dutch, mirrored from step-1/home.html) + design JSON + cache_version
├── lang/
│   └── nl/
...
│       │   └── partials/
│       │       ├── header.blade.php    # nav (desktop + mobile drawer + search overlay) from $c['header']; account area is AUTH-AWARE: guest → Inloggen/Aanmelden buttons, logged-in → user name + dropdown (native <details>, no JS) with Mijn account → /profile + Uitloggen (POST /logout); mobile drawer tile shows the name + full-width logout button
...
│       │       ├── footer.blade.php    # columns/social/contact/trust/payments from $c['footer']
...
### CMS data (seedered, 63 blocks + design)
...
- Footer: brand info, social (4), 3 link columns, contact (4), trust (3), copyright, payments (4)
...
- `pages.home.sections`: editable sections — `header` (logo_image upload, logo_text, tagline), `hero` (badge, 3 title lines, description, 2 images), `why` (badge, title_prefix, title_highlight, description, hub_icon/title/subtitle, `benefits` json, `stats` json with **`'columns' => 2`**). json blocks support an optional `'columns'` key → the editor renders the rows as a grid instead of stacked
...
| `x-admin.sidebar-link` | Sidebar nav link: `:active`, `icon` slot, optional `badge` slot |
