...
- [2026-08-15] — **Auto-deploy from GitHub (Hostinger)**: added `scripts/deploy.sh` (server-side, idempotent: `git fetch origin main` + `git reset --hard origin/main` — the server mirrors GitHub exactly; runs `composer install --no-dev --optimize-autoloader --prefer-dist`, `php artisan migrate --force`, `php artisan optimize:clear` ONLY when the code changed; logs to `storage/logs/deploy.log`, auto-truncated at 5 MB). A server cron (`*/5 * * * * /bin/bash .../scripts/deploy.sh`) pulls every 5 minutes — push to `main` is all that's needed to go live. OpenCode deploy skill `.opencode/skills/deploy/SKILL.md` + `/deploy` command `.opencode/command/deploy.md`. Docs in `DEPLOYMENT.md` (server facts, flows, troubleshooting). SSH: key `~/.ssh/slimmepc_deploy` added to the server's `authorized_keys`, `~/.ssh/config` alias `slimmepc` (Host 82.25.102.153, port 65002, user u439113944). App dir on the server: `/home/u439113944/domains/slimmepc.kulshy.online/public_html`. The SSH password lives ONLY in the gitignored `deploy-credentials.local.md` — deliberately not committed (public repo).
