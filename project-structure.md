...
- [2026-09-03] — **Webshop product-details pagina (exact product-details.html)**: nieuwe route `GET /webshop/{categorySlug}/{productSlug}` → `WebshopController@show` (`webshop.product`, vóór `webshop.category`), laadt `Category where slug+active` + `Product where slug+active+category_id` (404 indien inactief), `allCategories` voor chips/header, `relatedProducts` (zelfde categorie `inRandomOrder limit 4`). View `resources/views/landing/product-details.blade.php:1` erft `landing.layouts.app` + `header/footer` en kopieert het volledige `<style>` + `tailwind CDN slimme palette (#0757ef) + Inter + FontAwesome 6.5.2` uit `product-details.html` (reveal, product-stage breathe, thumbnail active, choice-badge float, heart, quick-spec hover, stock-dot pulse, cart-button shine, slimme-section, tabs, stickyCart, shop-card::after). Sections: breadcrumb `Home > Webshop > category > title`, gallery `product-stage min-h-[505px] rounded-[24px]` met `is_featured` badge `Slimme-PC keuze`, nav `previous/next`, `1/5` counter, thumbnails `grid-cols-5`, info `brand/category, title 30/36/40px, rating 4.8 (24), shortSpecs (•-joined), prijs € + oldPrice/discount% + Inclusief btw/SKU, voorraad `stock-dot` + `delivery_time || Voor 16:00…`, quick-spec 4 kaarten (nu highlights), qty + `In winkelwagen` + assurance (garantie/afhalen/veilig), `Geselecteerd door Slimme-PC` + `Persoonlijk advies nodig?`, tabs `Over dit product (description + highlights 4 kaarten) / Levering & garantie`, `Technische gegevens` (2-koloms grid uit `features` title: value, verdeeld over links/rechts), `Maak je setup compleet` (related 4). JS `updateImage/next/previous/selectImage/changeQuantity/toggleFavorite/addToCart/openTab/reveal/stickyObserver` identiek aan origineel. Koppeling: `landing/webshop.blade.php:650` title + `650 img` + `686 cart` werden `<a href="{{ route('webshop.product', [$product->category->slug, $product->slug]) }}">`.
- [2026-09-03] — **Seeder 8 volledig gedetailleerde producten**: `database/seeders/DetailedProductsSeeder.php` `updateOrCreate` 8 producten (5× Labtop `ASUS Vivobook 15 X1504ZA 699, HP 15s 599, Lenovo IdeaPad 3 549, Legion 5 1299, Dell Inspiron 15 749` + 2× Gaming `MSI Infinite S3 1299, Corsair Vengeance 1899` + 1× Onderdelen `Kingston NV2 1TB 79`) met `features [{title,value}×4]` + `highlights [{icon,title,subtitle}×4 lucide]`, `gallery_images[5] Unsplash`, `discount_type/value/dates`, `delivery_time`, TinyMCE `description`; oude demo `hp-2026/laptop1` verwijderd. `php artisan db:seed --class=DetailedProductsSeeder --force`.
- [2026-09-03] — **Product-details: lege velden verbergen + geen verzonnen labels**: `landing/product-details.blade.php:11` `deliveryText` zonder fallback, `shortSpecs` `null` i.p.v. `15.6"...`, `quickSpecs/extendedSpecs/specRows` alleen uit `features` echt ingevuld (geen `Processor/RAM`-hardcode, geen `512GB NVMe SSD` defaults, geen `Intel Core i5-1235U` fallback), `description` fallback verwijderd, `Technische gegevens/Meer over dit model` verborgen indien `features/description` leeg, `quick-spec grid` verborgen indien leeg, `extendedSpecs` rechts leeg (geen `Windows 11` defaults), `shortSpecs` paragraaf en `Snel in huis` delivery rij conditioneel. Validatie: leeg product `test-leeg-product` toont geen 512GB/1235U/tech.
- [2026-09-03] — **Product-details design exact + Snel in huis fixed**: `cdn.tailwindcss.com` + `slimme 50-900 + Inter + FA6.5.2` toegevoegd in blade (was missing → utilities als `slimme-600/shadow-soft` werkten niet), `body radial gradients` terug, `html scroll-behavior`, alle custom CSS 1:1. `Snel in huis` aside kreeg `items-start` op grid + `h-fit self-start` op aside i.p.v. `stretch`, zodat de donkere kaart niet meegroeit met `Over dit product`.
- [2026-09-03] — **Features → 2 kolommen (titel + waarde) + Highlights per product**: `products` blijft 1 tabel (geen nieuwe tabellen). Nieuwe migration `2026_09_04_000001_add_highlights_to_products_table.php` → `highlights json nullable after features`. `Product.php:18` casts `highlights=>array`. `ProductController@store/update` nieuwe validatie `features.*.title required max:80, features.*.value required max:255` + `highlights.*.icon nullable max:64, highlights.*.title required max:60, highlights.*.subtitle nullable max:80` (max 20 features, max 8 highlights) + backwards compat voor legacy `string[]` → `[{title:"Specificatie N", value:str}]`, filtering op lege rijen. `admin/shop/products/create.blade.php:275/edit.blade.php:304` volledig herbouwd: `Specificaties` nu `Titel | Waarde` grid `170px 1fr 40px` + `+ Specificatie toevoegen` → `addFeatureRow()`, `Highlights` nieuw `icon picker (data-icon-picker, lucide kebab, truncated) | Titel | Subtitel` grid `140px 1fr 1fr 40px` + `+ Highlight toevoegen` → `addHighlightRow()` (met `lucide.createIcons()`), `p-2.5 rounded-xl border`. AI generator in beide views aangepast om `features[title/value]` te verzamelen als `title: value`. Frontend `landing/product-details.blade.php:28` normaliseert beide formaten (`string[]` legacy → object), `shortSpecs` uit `featuresDisplay`, `quickSpecs` nu **uit eerste 4 `highlights`** (was `features`) met lucide/fa switch, `highlights` feature-mini onder `Over dit product` dynamisch `@foreach($highlights)` met `data-lucide/fa` + `title/subtitle`, secties verborgen indien leeg. `landing/webshop.blade.php:594` + `landing/product-details:436` related `implode` gefixt voor `object[]` (was `Array to string conversion`), webshop fallback `15.6"...` verwijderd (leeg indien geen features). Highlights seeder voor alle 8 producten toegevoegd.
- [2026-09-03] — **Dashboard Specificaties grid fix + Specificaties tabel hersteld**: `create/edit` grids `grid-cols-[...]` arbitrary werkten niet (JIT mist JS strings → viel terug naar 1 kolom, Titel leek checkbox). Omgezet naar `style="grid-template-columns: 170px 1fr 40px;"` en `140px 1fr 1fr 40px` inline, `w-10 h-10` delete button, `truncate` op icon name. `landing/product-details` onderste tabel `Technische gegevens` hersteld naar originele 2-koloms `Processor / Werkgeheugen / Opslag / Scherm / Videokaart` vs `Besturingssysteem / Draadloos / Poorten / Gewicht / Kleur` en daarna teruggebracht naar dynamische variant: alleen echte `features` verdeeld over `md:grid-cols-2` (`Processor: Intel...` · `Werkgeheugen: 16GB...`), `Besturingssysteem` etc. volledig verwijderd (was statisch), `@if(!empty($specs))` en `Meer over dit model` kaart definitief verwijderd per client, `Technische gegevens` alleenstaand full-width wanneer `highlights` leeg.
- [2026-09-03] — **Tabs Specificaties verwijderd + onderste sectie terug**: `product-details.blade.php:310` tabs `grid-cols-3` → `grid-cols-2`, middelste knop `Specificaties` + `tabSpecs` div verwijderd; onderste `Technische gegevens / Meer over dit model` sectie eerst verwijderd, daarna op verzoek alleen `Specificaties` teruggezet (zonder `Meer over dit model`), als single-column full-width card.
- [2026-09-04] — **Specificaties tabel definitief dynamisch 2-koloms**: `Besturingssysteem / Windows 11...` statische rechterkolom verwijderd per client, `product-details.blade.php:46` nu `specs[]` uit `features` (`title !== '' ? title : Specificatie`) en weergave `@foreach($specs)` in `grid md:grid-cols-2` — bij 2 Specificaties staat er 1 links, 1 rechts in dezelfde rij; bij 0 is de hele sectie verborgen.
- [2026-09-04] — **Huidige verzoek — Specificaties vs Quick cards gescheiden**: `product-details.blade.php:48` `shortSpecs` onder titel = **eerste 4 `Specificaties` (features `title: value` joined `•`)**, `quickSpecs` 4 kaarten onder `Op voorraad` = **eerste 4 `Highlights` (icon + title/subtitle)** i.p.v. Specificaties. `quickSpecs` rendering kreeg lucide/fa switch (`data-lucide` vs `fa-solid`). `curl http://127.0.0.1:8000/webshop/labtop/asus-vivobook-15-x1504za → 200` bevat `Processor: Intel` in regel en `Lichtgewicht` in kaarten.
- [2026-09-04] — **Over dit product — 4 regels + Meer weergeven**: `product-details.blade.php:318` description nu in `#descWrapper max-height:112px (≈4×28px) overflow-hidden` met `product-desc-content` CSS exact als TinyMCE `content_style` (Figtree 14px/1.7, h2/h3, ul/disc, li, strong, em, a, blockquote, img, code) i.p.v. `text-[13px] prose`, `+ #descFade gradient`. Knop `#descToggle Meer weergeven ↔ Minder weergeven` met `fa-chevron-down/up`, JS meet `scrollHeight > 120`, toont knop alleen bij lange tekst, `resize` handler, smooth `max-height` expand/collapse `.35s`, `scrollIntoView nearest`.
- [2026-09-04] — **WYSIWYG exact behouden**: `product-details.blade.php:127` nieuwe CSS `product-desc-content` kopieert TinyMCE `content_style` 1:1 (ul/ol/disc/decimal, h2/h3, p, strong, a, img, code, blockquote), wrapper klasse `text-[13px] prose` verwijderd, raw `{!! $product->description !!}` binnen `#descContent.product-desc-content` toont exact zoals in editor gebouwd (geen Tailwind preflight stripping).
- [2026-09-04] — **Highlights & Specificaties dashboard volledig dynamisch (huidig)**: `ProductController` validatie uitgebreid, `create/edit` volledig herbouwd naar `170px/140px` grids, seeder her-seeded, `product-details` highlights dynamisch met lucide.
- [2026-09-05] — **Dashboard Specificaties inline-grid fix**: `grid-cols-[...]` arbitrary faalde (JIT mist JS strings → 1 kolom, Titel leek checkbox). Omgezet naar `style="grid-template-columns: 170px 1fr 40px;"` / `140px 1fr 1fr 40px` inline in `create/edit.blade.php:289/302` + JS `addFeatureRow/addHighlightRow` met `style.gridTemplateColumns`, `h-10 w-10` delete, `truncate` op icon-picker. Bestaande rijen + nieuwe rijen nu horizontaal correct.
- [2026-09-05] — **Technische gegevens terug naar origineel (2-koloms)**: `product-details.blade.php:362` `Technische gegevens` hersteld naar originele 2-koloms `md:grid-cols-2` met `Processor/Werkgeheugen/Opslag/Scherm/Videokaart` vs `Besturingssysteem/Draadloos/Poorten/Gewicht/Kleur` en daarna omgezet naar **dynamisch** `specs[]` uit `features` verdeeld over 2 kolommen (`@foreach($specs)`), statische rechterkolom verwijderd, `@if(!empty($specs))`.
- [2026-09-05] — **Specificaties vs Quick-cards gescheiden + WYSIWYG 4 regels**: `shortSpecs` (onder titel) = eerste 4 `Specificaties` (`features title: value •`), `quickSpecs` (4 kaarten onder `Op voorraad`) = eerste 4 `Highlights` (`icon/title/subtitle`) met lucide/fa switch. `Over dit product` description in `#descWrapper max-height:112px` + `#descFade` + knop `Meer weergeven ↔ Minder weergeven` (meet `scrollHeight > 120`, `resize` handler, smooth `.35s`). `product-desc-content` CSS exact als TinyMCE `content_style` (geen `prose` stripping) — `{!! $product->description !!}` toont exact zoals in editor.
- [2026-09-05] — **Reviews systeem (gast + auth, geen limiet, alleen rating in front)**: nieuwe migration `2026_09_04_000002_create_product_reviews_table.php` (`product_reviews` + `products.rating_avg 3,2 + rating_count + index`), `ProductReview` model + `Product::reviews()/approvedReviews()/recalcRating()`. `WebshopReviewController@store` `POST /webshop/{cat}/{prod}/reviews` `throttle:5,1` — gast: `guest_name/email` required, auth: `guest_name` required (vervangt `Titel (optioneel)` → `Naam`), `is_approved=true` + `recalcRating()` direct. Frontend `product-details.blade.php:242` rating dynamisch `avgRating/ratingCount` met `fa-star/half` + `Schrijf een review` → `openReviewModal()` (`#reviewModal z-[70]` backdrop-blur, `rounded-[20px]`, `Inter`, `starPicker` 5×, `body` min 10, toast). `webshop.blade.php:628` rating op kaarten dynamisch (`★` op basis van `avg`). Admin `Admin\Shop\ReviewController@data/approve/reject/destroy/productReviews` + `routes/admin.php:203` `admin.webshop.reviews.*`, `resources/views/admin/shop/products/index.blade.php:131` kolom `★ Beoordeling` + `modal-productReviewsModal` (gemiddelde/goedgekeurd/totaal + `Goedkeuren/Afkeuren/Verwijderen`), `public/assets/js/admin/products.js:72` 11→12 cols, `ratingCell` met `data-reviews`, `openReviews()` + `recalc` na acties.
- [2026-09-05] — **Fixes: Naam i.p.v. Titel, Slimme-PC Beheerder, lege producten**: `product-details` review modal `Titel (optioneel)` → `Naam *` (value leeg i.p.v. `auth()->user()->name` prefill `Slimme-PC Beheerder` verwijderd), `ProductReview::getAuthorName` nu `guest_name` eerst, `guest_name` required voor gast+auth. Admin `products.js` syntaxfout `}));` + `AdminTable.loading(11→12)` + `colspan 11→12` gefixt (was `Totaal: 0` leeg), `view:clear/cache:clear`. Pending reviews (2× Dell) `is_approved 0→1` + `recalcRating` (5.00/2). `Snel in huis` card `items-start + h-fit self-start` i.p.v. stretch.

## 22. Webshop Product Details & Reviews Systeem (2026-09-05)

### Product Details Route
- `GET /webshop/{categorySlug}/{productSlug}` → `WebshopController@show` (`webshop.product`)
- `POST /webshop/{categorySlug}/{productSlug}/reviews` → `WebshopReviewController@store` (`webshop.reviews.store`, throttle 5,1, CSRF, gast+auth)

### Reviews Flow
- Popup `Schrijf een review` (zelfde design systeem: `Inter`, `slimme` gradient, `rounded-[20px]`, `starPicker`, `Naam*` + `E-mail*` voor gast, `Naam*` voor auth, `body` 10-1000) → `201 {message, review}` → `is_approved=true` direct → `Product::recalcRating()` → `rating_avg/count` getoond in `product-details` (sterren + `4.8 (n)`) en `webshop` kaarten (`★★★★★ (n)`).

### Admin Beheer
- `Admin\Shop\ReviewController` 4 endpoints (`data` met search/rating/status/product filter, `approve`/`reject`/`destroy` + `recalc`), `productReviews(product)` voor modal.
- `admin.webshop.reviews.*` onder `admin` middleware.
- `admin/shop/products/index` kolom `Beoordeling` (11→12 cols) met `★ avg (count)` knop → modal met pending/approved + acties.
...
## 30. Update 2026-09-13 — Live Chat fase 1 (welkom → FAQ-AI + openingstijden + offline-mail + rating)

- **Doel:** chat opent met welkomstbericht eerst; Kennisbank + Openingstijden in dashboard; AI antwoordt op basis van FAQ's. Inbox-spiegel, Pusher en productkaarten volgen later.
- **Database (`2026_09_13_000001_create_chat_tables.php`, migrate DONE):** `chat_faqs` (question/answer/category/is_active/sort_order + index), `chat_availability` (day_of_week 0–6 unique, is_open, open_at/close_at — seeded ma–vr 09:00–17:00, za 10:00–14:00, zo dicht), `chat_closed_dates` (closed_at unique + reason), `chat_conversations` (guest_token 64 unique, user_id FK nullOnDelete, name/email, status ai/open/handed_over/closed/offline, ai_enabled, handed_over_at, admin_read_at, last_activity_at, rating 1–5 + rating_comment + rated_at, ip + indexen), `chat_messages` (FK cascade, sender customer/admin/ai, body, attachment, source widget/dashboard/email/ai).
- **Models:** `ChatConversation` (user/messages/touchActivity) + `ChatMessage` + `ChatFaq` (scopeActive) + `ChatAvailability` (dayName NL) + `ChatClosedDate`.
- **Availability (`App\Services\Chat\ChatAvailabilityService`):** alles in **Europe/Amsterdam** (app-timezone is UTC — coupon-valkuil); vrije datums > weekdag-rij > tijdvenster; `nextOpening()` max 14 dagen vooruit. `GET /ai-chat/status` → `{open, reason, opens_at}`.
- **Admin (`admin.chat.*`, 11 routes + sidebar-dropdown "Live Chat"):** `Chat\FaqController` (index/data/store/show/update/destroy/toggle — Hardware-stijl tabel + modals + `chat-faqs.js`) + `Chat\AvailabilityController` (weekdag-rijen met toggle+tijden+save, vrije-dagen add/delete) + `ChatFaqSeeder` (5 NL defaults, `firstOrCreate`, in `DatabaseSeeder`).
- **Publieke API (`ai-chat.*`, 10 routes, throttle per endpoint, honeypot `website`):** `ChatController` — status/start (ingelogd: user_id + naam/e-mail van account, gast: validatie; altijd eerst AI-welkomstbericht in DB)/send (tekst en/of 1 foto 10MB → `chat/{id}/` local; AI-antwoord via `ChatAnswerGenerator` bij ai/open + ai_enabled; ai→open bij eerste bericht)/messages (poll)/history (alleen auth, eigen gesprekken)/handover (ai stopt + notify-mail)/offline (gesprek offline + 2 mails)/close/rate (1× per gesprek)/photo (stream met token-guard). **CSRF-exempt `ai-chat/*`** in `bootstrap/app.php` (zelfde reden als `contact/submit`: full-page HTML-cache serveert stale tokens; echte auth = 64-char guest_token + throttle + honeypot).
- **AI (`Ai\Features\ChatAnswerGenerator`):** actieve FAQ's via **score + keywords + Arabische normalisatie** (woorden ≥3, alef/ة/ى/diakrieten genormaliseerd, top 3 op score + sort_order) + laatste 10 berichten als context → `AiService::chat()` (temperature 0.5, max_tokens 400) met Slimme-PC system-prompt + TAALREGEL (alleen laatste klantbericht bepaalt de taal); fallback-NL bij LLM-fout. `chat_faqs.keywords` (migration `2026_09_13_000002`, NL+AR triggers, dashboard-editable) overbrugt de taalkloof AR-vraag → NL-antwoord.
- **Kennisbank-fix 2026-09-13:** AR-vragen ("كيف بقدر اتوصل معكم") matchten nooit (LIKE op NL-tekst; "اتوصل" ≠ "تواصل"). Nu: keywords met AR-varianten + normalisatie + scoring. Geverifieerd live: "كيف بقدر اتوصل معكم" → antwoord mét 055 203 21 45 + info@slimme-pc.nl + WhatsApp, in het Arabisch.
- **Mails (design-systeem `emails/layout`, synchronous):** `ChatOfflineReceived` (klant-bevestiging, Reply-To `+chat-{id}@` zoals contact) + `AdminChatNotification` (offline/handover naar `CONTACT_NOTIFY_EMAIL`, CTA nu dashboard — deep-link volgt met inbox-fase).
- **Widget (`ai-chat.blade.php` + `ai-chat.js` herschreven):** open → status-check → open: statisch welkom + **gate (naam/e-mail, prefill bij login)** → start → server-berichten + suggesties + input + paperclip (preview + 10MB-check) + actiebar (Medewerker spreken / Beëindigen) + polling 15s (alleen open paneel) + history-overlay (alleen login) + rating-view (sterren + comment + Overslaan, na close); gesloten: offline-formulier + reden + opens_at. `window.AiChat`-config (user + routes) inline in partial.
- **Verification:** status-endpoint 200 (zo dicht → `opens_at ma 09:00+02:00`); start 201 + welkom; send 201 + FAQ-gegrond AI-antwoord (live OPENAI-key); handover/close/rate 200; mails renderen (layout + handover-tekst); `view:cache` ok; testrijen opgeruimd (0 gesprekken, 5 FAQs blijven). Gotcha gevonden: 419 door stale CSRF-token uit page-cache → `ai-chat/*` exempt.
- **Fix 2026-09-13 — echo/duplicaten + taal + RTL:** widget toonde berichten dubbel (optimistische klant-bubbel + statische welkom werden niet in `renderedIds` geregistreerd → poll rendeerde ze opnieuw; leek alsof AI het eigen bericht herhaalde). Opgelost met `renderThread()` (herbouw vanaf server, registreert alle ids, suggesties alleen zonder klantbericht; optimistic-id wordt bij send-respons geregistreerd). DB bewezen schoon (5 rijen: welkom+2 klant+2 AI). Verder: `dir="auto"` op alle bubbels (Arabisch RTL) + system-prompt antwoordt nu **in de taal van de klant** (geverifieerd: Arabische input → Arabisch antwoord). Handover-flow: knop → `handed_over` + AI stopt + notify-mail naar `CONTACT_NOTIFY_EMAIL`; admin-inbox (overname-knop + deep-link) volgt in fase 2 — tot dan via e-mail.
- **Fix 2026-09-13 — gesloten-gesprek herstel:** send op gesloten/offline-thread gaf 422 "Dit gesprek is gesloten" zonder uitweg (dode token in localStorage, gate permanent weg na rating). Nu: server stuurt `code: conversation_closed` → widget reset automatisch naar verse gate (`resetToFreshGate()` herbouwt gate + melding) na send-422, na rating (skip/verstuur) en na resume-faal; resume op `offline` toont offline-view i.p.v. chat-input. Geverifieerd: start → close → send = 422 + code.
- **Fix 2026-09-13 — status-refresh bij elk openen:** widget checkte open/dicht één keer per pageload (`booted`) + 60s cache → na het opslaan van uren bleef "Gesloten" staan tot refresh. Nu: status-check bij elke open (15s cache) + `refreshOnOpen()` upgrade offline → open automatisch (downgrade van actief gesprek nooit; gesloten threads via 422-reset). Geverifieerd: ma 19:06 binnen 03:00–23:59 → `{"open":true}`.
- **Fix 2026-09-13 — offline-token bleef offline tonen:** token van een oude offline-thread dwong altijd de offline-view af, ook toen de shop open was (offline-threads kunnen in de widget niets versturen). Nu: resume() op `offline` checkt de actuele status → dicht = offline-view, open = verse gate ("We zijn weer open!").
- **UX 2026-09-14 — site-design confirms + rating-sluiting:** alle native `confirm()`'s vervangen door `window.SlimmePC.confirm` (zelfde modal als winkelwagen-verwijderen: `ai-chat` handover/beëindigen + ontvangst-foto-delete met native fallback). Rating-view sluit het paneel niet meer abrupt: na Versturen/Overslaan → bedankje (★-echo) + knop "Nieuw gesprek starten" (pas dan verse gate). Diagnose-hulp: `window.AiChat.version` in console (`2026-09-14e`) bewijst verse JS (stale-cache was bij elke "nog steeds stuk"-melding de oorzaak).
- **Fix 2026-09-14 — dubbele sends + Arabische productbrug:** sluitstuk van het "twee verschillende antwoorden"-mysterie: Enter/CTTRL dubbel-klik kon 2 sends afvuren (temperatuur-verschil → ander antwoord). Nu `sending`-flag (client) + temperature 0.5→0.3 + verplichte productnoeming ("MOET minimaal één noemen, nooit 'weet niet' bij producten"). Dieper probleem gevonden: "لابتوبات بسعر 600 يورو" gaf 0 kandidaten (meervoud + يورو niet herkend + NL-titels nooit AR-match) → nieuwe `App\Services\Chat\ArabicText` (normalisatie + meervoud ات/ين/ون eraf + AR→NL termen-map) gebruikt door FAQ-scoring én product-zoekopdracht; budget-regex + `يورو`; budget-only fallback zonder woordmatch. Geverifieerd live: exacte query → Arabisch antwoord + HP 15s €599 + echte link + kaart, handoff=false. Les: bij "technisch mis"-fallback eerst `laravel.log` checken (vond DNS-storing api.aimlapi.com — correcte fallback).
- **Fix 2026-09-14 — widget-bugs + slimmere bot:** (1) Medewerker-bubbels rechts (wit met blauwe ring + label, i.p.v. links tussen AI). (2) Confirm-modal viel achter paneel (z-60 vs z-1000) → tijdelijk z-1100, dus altijd zichtbaar. (3) Klik op confirm-modal sloot het chatpaneel (outside-click handler) → `[id^="modal-"]`-kliks genegeerd. (4) Bot escaleerde alles ("waarmee help je" → out-of-scope + handoff): nieuwe capabilities-FAQ (NL+AR+EN keywords, sort 0) + prompt-regel dat hulpvragen géén handoff zijn. Geverifieerd: "طيب شو بتساعدوني ؟" → diensten-overzicht in het Arabisch zonder handoff; onzin → handoff_offer=true.
- **Volgende fasen:** admin inbox-spiegel (threads + badges + AI aan/uit + deep-link in notify-mail) → Pusher realtime (polling blijft fallback) → productkaarten (`discounted_price` + `webshop.product`-links) → e-mail-threading (`+chat-{id}` in `InboundContactFetcher`) → `.env.example` aanvullen (`OPENAI_*`, later `PUSHER_*`).

## 31. Update 2026-09-13 — FAQ embeddings (hybrid retrieval) + AI keyword-generator

- **Keuze (op verzoek):** embeddings via `EMBEDDING_MODEL="openai/text-embedding-3-small"` × `EMBEDDING_DIMENSIONS=1536` (AIML, OpenAI-compatibel). MySQL is **5.7** (géén pgvector) → vectoren als **JSON** in DB + cosine in PHP (bij ≤100 FAQs ~ms, géén extra infra).
- **Config:** `services.embedding` (key hergebruikt van `OPENAI_API_KEY`; `api_url` = `EMBEDDING_API_URL` of **afgeleid** `dirname(chat_url)/embeddings` → `api.aimlapi.com/v1/embeddings`; model/dimensions/timeout). `.env.example` aangevuld met `OPENAI_*` (oude gap) + `EMBEDDING_*`. Gotcha gevonden: eerste afleiding gaf `/v1/chat/embeddings` (404) → nu `/chat/completions` → `/embeddings` vervangen.
- **Client:** `Contracts\EmbeddingClientInterface` + `Clients\OpenAiEmbeddingClient` (POST `{model,input}`, 8000-char trim, dim-check, NL-errors) + `AiService::embedding()/setEmbeddingClient()/embed()`.
- **Opslag:** migration `2026_09_13_000003` (`chat_faqs.embedding` JSON + `embedding_model`). Gotcha gevonden: backfill "Embedded: 5" maar kolommen leeg — `embedding` ontbrak in `$fillable` (stil genegeerd) → toegevoegd + `--fresh` rerun. Auto-embed bij FAQ store/update via `afterResponse` (`FaqEmbedder::embedFaq`); backfill-commando `php artisan ai:embed-faqs [--fresh]`.
- **Retrieval (`ChatAnswerGenerator`, hybride):** embed query → cosine vs actieve FAQs (zelfde model) → top-3 met sim ≥ 0.30 + **+0.15 keyword-bonus** (exacte triggers zoals telefoonnummers) → bij falen/geen vectoren **fallback keyword-scoring** (normalisatie + keywords). Geverifieerd live: AR-contactvraag → contact-FAQ; EN-parafrase "My device is dead, how long" → duur-FAQ; NL-parafrase → contact-FAQ.
- **Keyword-generator (op verzoek, NL+AR+EN):** `POST /admin/chat/faqs/generate-keywords` (zelfde patroon als `AiProductController`, throttle) + ✨-knop naast Keywords-veld (loading, confirm bij overschrijven, toast). Geverifieerd: contact-vraag → `telefoon, nummer, contact, تواصل, اتصل, phone, number…`.
- **Tests:** `tests/Unit/ChatEmbeddingTest.php` (4 tests: cosine-math, embedding-pick met mock, fallback bij API-faal, fallback zonder vectoren) — **4/4 pass**.
- **Deploy-nota:** na push op server `migrate --force` + eenmalig `php artisan ai:embed-faqs` (deploy.sh doet alleen migrate).

## 33. Update 2026-09-14 — Chat↔e-mail rondje rond (foto-fix + ticket-transcript + always-mail + inbound + lock)

- **Bugfix foto (oorzaak bewezen):** admin-tak in `ai-chat.js renderMessage` rendeerde alleen body en negeerde `photo_url` (backend stuurde hem wel) → foto's van medewerkers nooit zichtbaar (ook niet na refresh). Nu: foto onder Medewerker-bubbel rechts (zelfde patroon als klant). Live geverifieerd: admin-reply met foto → 200 + `photo_url` in widget-payload + stream 200 + bestand op disk.
- **Ticket-mail = berichten van de klant:** `AdminChatNotification` (handover) toont nu laatste max-3 klantberichten + meta + deep-link; zelfde `CONTACT_NOTIFY_EMAIL`.
- **Admin-reply mailt altijd:** `InboxController@reply` stuurt nu altijd `ChatReplyMail` (was alleen offline) + `attachments()` met reply-foto; AI stopt (`ai_enabled=false`) + ai→open-promotie; **lock:** reply op `closed` → 422 "Ticket gesloten — heropen…".
- **Inbound `+chat-` (spiegel contact):** `InboundContactFetcher` tweede pass (`to('+chat-')`, regex `\+chat-(\d+)`, `cleanBody` + `[image:]`-scrub, eerste bijlage naar `chat/{id}/inbound`, fail-voor-rij-garantie, `Seen`) → `ChatMessage` (customer/inbound) + `touchActivity` + closed/offline → heropen (`open` bij open winkel). **Geen cron**: rijdt mee op `inbound.sync` + uitgebreide `POST admin/chat/inbox/sync` (15s-throttle zoals contact) + handmatig commando.
- **Lock-UI:** inbox toont **"Ticket gesloten"**-banner + disabled composer bij `closed` (ook client-guard met toast); widget had al `conversation_closed`-reset.
- **Bedankmail bij sluiten:** nieuwe `ChatClosedMail` ("Bedankt + transcript laatste 10") bij overgang naar `closed` vanaf beide kanten (widget `close` + dashboard `status`), Reply-To `+chat-{id}` (re-open mogelijk), eenmalig per overgang.
- **Tests:** `ChatInboxTest` +4 (lock-422 zonder mail; thanks-mail exact 1× per overgang beide kanten — les: terminating-callbacks vuren per request opnieuw in tests → één mail-request per test; inbound-MIME → rij + reopen; handover-transcript) — **19/19** met ChatEmbedding. Live letsel: testthreads + `chat/*`-mappen opgeruimd.
- **Fix 2026-09-15 — inbound foto onzichtbaar:** `storeChatAttachment()` bewaarde bestand onder `chat/{id}/inbound/naam` maar gaf `'inbound/naam'` terug (contact-versie plakt prefix er pas bij `attachment()`-lezen bij; chat-readers gebruiken het pad direct) → alle foto-URL's 404. Nu volledig pad (zoals outbound/uploads). Live bewezen: echte foto-mail → `chat/157/inbound/scherm.webp` + bestand bestaat + widget-stream 200 + admin-payload `photo_url`. Test `storeChatAttachment` op volledig pad. **Tests 19/19**.
- **Fix 2026-09-14 — medewerker-antwoord vanuit eigen mailprogramma:** `AdminChatNotification` kreeg `Reply-To: +chat-{id}` (ontbrak — replies van de medewerker gingen nergens heen). Fetcher herkent eigen adressen (`notify_email`/`mail.from`/`imap-username`) → `sender=admin, source=email` + `ai_enabled=false` + doorsturen `ChatReplyMail` naar klant + reopen bij gesloten. Handover-ticketmail droeg al transcript; **live bewezen**: echte Gmail `+chat`-reply → `processed=1 matched=1` → rij in DB + zichtbaar in widget/inbox. Les uit tests: relatie-default `orderBy('id')` + `orderByDesc` in test = `ORDER BY id ASC, id DESC` (ASC wint!) → in tests `reorder()` eerst gebruiken; productiecode was correct.
- **Fix 2026-09-15 — lege CTA-knop in ticket-mail:** `@section('cta_label')Open gesprek@endsection` op één regel compileert niet (zelfde familie als §25-bug) — `@endsection` lekte letterlijk in de HTML vóór de layout, knop zonder tekst. Alle 4 chat-mailviews naar multiline `@section/@endsection`. `EmailDesignTest`: guard uitgebreid met `not->toContain('@section'/'@endsection')` + nieuwe test rendert alle 4 chat-mails (CTA-tekstpipeline geverifieerd) — **11/11**.
- **Fix 2026-09-15 — stille mail-sterfte:** `getaddrinfo failed` voor smtp.gmail.com in de log bewees het: één haperende SMTP-verbinding gooide de hele afterResponse-closure weg, dus vervolgmails (admin-notify na klantmail!) kwamen nooit aan — zonder zichtbare fout (201 + geen log bij de gebruiker). Nieuwe `App\Services\Chat\ChatMailer::send()` (per mail try/catch + 1 retry na 3s bij transportfout + warning-log, gooit nooit) gebruikt in alle 5 chat-dispatches (handover×2, offline×2, close, inbox-reply, inbox-close). Live bewezen: handover → klant-ticketmail + admin-notify **beide** in echte Gmail.
- **Fix 2026-09-15 — gate-e-mail leidend (ook ingelogd):** `start()` + widget negeerden de gate bij login (`$user->email ?? input` + `authedUser.* ||`): alle mails gingen naar het accountadres (`slimmepc@admin.com` = geen echte inbox → verdampen na SMTP-accept!). Nu: gate-input altijd leidend + verplicht voor iedereen (prefill blijft als gemak), account alleen fallback bij leeg, `user_id` blijft gekoppeld. Offline was al correct. Tests: ingelogde start met afwijkende gate-mail + validatieplicht. Live bewezen: ingelogde admin + gate-mailbox → beide handover-mails in echte Gmail.
- **Fix 2026-09-15 — keyword-taaldetectie weg, AI schrijft zelf:** `ticketMessage()` + `offlineThanks()` in `ChatController` gebruikten regex-woordenlijsten (AR-schrift + EN-woorden) — op verzoek volledig vervangen door `ChatAnswerGenerator::confirmation($conv, 'ticket'|'thanks')`: AI schrijft bevestiging in klanttaal o.b.v. geschiedenis (fallback NL). Les: instructie als laatste user-bericht trekt taal naar NL + ai-ticketbericht in historie kleurt thanks → opgelost via expliciete override + **volgorde**: thanks genereren vóór ticketbericht (alleen klantbericht in historie). Offline = e-mail-only (geen token/thread terug, widget toont server-`thanks`). Handover stuurt nu ook `ChatTicketMail` naar klant (zelfde AI-tekst, Reply-To `+chat`) — live bewezen in echte Gmail.
- **UX 2026-09-15 — input + ticket-taal via citaat + intent:** photo-preview BOVEN het inputveld (was eronder) + `#aiChatInput` omgezet naar autogrow-textarea (Enter=versturen, Shift+Enter=nieuwe regel, max 120px, reset na send). Ticketbevestigingstaal via **citaat** (laatste klantbericht letterlijk in instructie, geen detectiecode — bewezen AR/EN/NL). Intent + `product|producten|منتجات|منتج`, typo-map `لالبتوب→laptop`, `contextBudget()` ook bij kaal getal + huidige productintentie ("منتجات بسعر 500", "بدود 1000"). Live bewezen: handover → klant-ticketmail + admin-notify beide in Gmail; admin-reply → klantmail met foto; inbox + attachments opgeschoond.

## 32. Update 2026-09-14 — Slimme chat (producten + CMS + conditionele handoff) + Inbox-spiegel

- **Producten (`App\Services\Chat\ChatProductSearch`):** live zoeken in actieve `products` (title/brand/sku/description LIKE) + **budget-extractie** (`€600` → bereik 0.5x–1.5x, sortering op dichtstbijzijnde prijs per afspraak) + in_stock eerst. Payload per kaart: titel/merk/`discounted_price`/old_price/stock/delivery/rating/image + **echte `webshop.product`-URL** (nooit verzonnen). Geverifieerd: "laptop rond 600 euro" → HP 15s €599 + echte link; "onder 500" → eerlijk "niet beschikbaar" + dichtstbijzijnde alternatief.
- **CMS-context (`App\Services\Chat\ChatCmsContext`):** compacte context uit `Cms::page()` (diensten + links, contactmethoden + reparatie/afspraak/tarieven-URL's, webshop-categorieën) gecachet op CMS-versie → AI kopieert **echte links exact** (linkify in widget maakt ze klikbaar).
- **Handoff-aanbod (`[handoff_offer]`):** prompt eist marker als AI het echt niet weet + aparte out-of-scope-regel; `looksClueless`-fallback (NL/AR/EN). Controller stript markers, valideert product-ids tegen payload (kaarten ook zonder markers: top-3). Geverifieerd: onzin-vraag → handoff_offer=true + knop.
- **Ticket-flow (op verzoek: zelfde gesprek):** handover → `handed_over` + `ai_enabled=false` + AI-wachtbericht ("Ticket aangemaakt… even geduld") + notify-mail (CTA nu deep-link `admin.chat.inbox.index?conversation=`).
- **Widget:** Beëindigen → header (phone-off-icoon + confirm; uit actiebar verwijderd); Medewerker-knop **verborgen tot `handoff_offer`**; links klikbaar (linkify); productkaarten (foto/prijs/stock/delivery + "Bekijk"-link).
- **Inbox (`admin.chat.inbox.*`, 10 routes, spiegel van contact-inbox):** full-height 2-panelen + zoeken/status-filter/paginatie + sortering laatste activiteit + **unread via `admin_read_at`** + beep + 30s-sync (slaat typen over) + `?conversation=`-deeplink + mobiele back-knop. Thread toont **klant/AI/medewerker-bubbels** (AI met label), foto's inline, account-badge (naam + klantnummer / Gast). Header: ★-rating (+ comment-chip), **AI aan/uit-toggle**, status-select, delete. Lijst: status-badges + ★ + unread-bubbel; header-counts (Open/Medewerker/Ongelezen/★-gemiddelde); sidebar-badge = handover-count. **Gouden regel: admin-reply → `ai_enabled=false`** (+ ai→open promotie); e-mail (`ChatReplyMail`, Reply-To `+chat-{id}@`) alleen bij offline-threads (realtime voor open threads; inbound-threading volgt later).
- **Ratings in inbox (op verzoek):** ★ per thread in lijst + header + comment-chip (alleen na close + rating door klant).
- **Tests:** `tests/Feature/ChatInboxTest.php` (7 tests: render+guest-redirect, data/unread/counts, show+read+rating, reply→AI-uit, toggle-ai, product-URL) — **7/7 pass**; suite ChatEmbedding+ChatInbox+AiService **14/14**. Gotchas: `actingAs` blijft binnen één test (aparte `it` voor guest-redirect); testdata zonder "laptop" in titel → lege search.
- **Fix 2026-09-14 — slimme antwoorden (geheugen + vergelijking + Arabische brug):** klachten: pitch-herhaling i.p.v. samenvatting ("waar hadden we het over"), één vast product ("geef een andere" genegeerd), specs-vraag → pitch, maps-link → handoff, budget-typo zonder productwoord → niets. Oplossingen: (1) `ArabicText`-helper (normalisatie + meervoud ات/ين/ون + AR→NL termen-map) gedeeld door FAQ-scoring én product-zoekopdracht; budget-regex + `يورو`; budget zónder woorden → prijsbereik-unie (altijd keuze i.p.v. 1 product). (2) `wantsDifferent` → getoonde slugs uit historie uitsluiten; `wantsSpecs` → `specsBlock()` (volledige features/highlights van laatst genoemd product); `wantsMemory` → trailing system-steer (samenvatten, taal van klant, nooit pitchen/handoff). (3) Vergelijkingen (`goedkoopste/duurste/ارخص`) deterministisch: PHP berekent winnaar → verplichte openingszin. (4) CMS-context + `locatie.route_url` (maps) — gotcha: 1-maand context-cache was vóór de wijziging gebouwd → `CACHE_VERSION`-constante voor code-busts. (5) Handoff-vlag = alléén `looksClueless()` (losse markers genegeerd) + verbod op "alleen Slimme-PC-vragen"-zin bij gronding + `stripTailOffers()` knipt bij gronding medewerker-/vleierij-staartzinnen weg (feiten met cijfers/links blijven) tegen LLM-variantie. **QA-matrix (12 checks: dummy/gewoon/AR/technisch): eerst 8/14 → na fixes 13/14 → na steering+strip 14/14** (incl. "مواصفات هاي اللابتوب" → echte specs, "عن شو حكينا" → Arabische samenvatting, maps → google.com/maps-link, cap-query 5/5 stabiel).
- **UX 2026-09-14 — links + minder geklets + context-budget:** lange URL's braken uit de bubbel → `linkify` toont korte tekst (max 42 chars + `…`, volle URL in tooltip) + `overflow-wrap:anywhere`-CSS in partial. AI plakte elke keer "hoe kan ik je helpen / meer informatie nodig" eraan → promptregel "max 3 zinnen, NOOIT slotvragen" (geverifieerd weg). "طيب بحدود 1000" (kaal getal, na 600-gesprek) gaf niets terug (herhaalde 600-pitch) → `contextBudget()`: kaal 3–4-cijferig getal + shopping-historie = nieuw budget (geverifieerd: 1000 → kaarten €500–1500, geen handoff). Suggestie-chips onder welkom (Laptop/Prijzen/Contact) verwijderd op verzoek (+ JS-referenties opgeruimd). Handover: typing-dots tijdens ticket-aanmaak + ticketbericht in klanttaal (`ticketMessage()`: AR bij Arabisch schrift, EN bij Engelse signaalwoorden, default NL) — geverifieerd alle 4 varianten.
- **UX 2026-09-14 — FAB + geluid:** floating AI-knop icoon `message-circle` → custom 3-strepen SVG (witte afgeronde bars, als voorbeeld) + swap naar X bij open paneel (`setFab`, terug bij sluiten). Geluid via Web Audio zonder bestanden (zelfde oscillator-patroon als inbox-beep): entry-chime bij binnenkomst (1,2s; bij autoplay-blokkade na eerste interactie), blip bij versturen, twee-toon ding bij AI-/medewerker-antwoord (ook via poll), mute-toggle in header (volume-2/volume-x, `localStorage slimmepc-chat-sound`). `renderMessage()` retourneert nu bool voor ding-detectie.
- **UX 2026-09-15 — lightbox + loading + snellere medewerker:** chatfoto's openen in popup binnen het paneel (`#aiChatLightbox`, `data-photo`-anchors, Escapevoorrang) i.p.v. nieuw tabblad; geluidsicoon verborgen (altijd aan); input al h-11-gelijk aan knoppen; spinner-loading (`btnLoading`) op versturen/starten/handover/beoordelen (offline had al); poll adaptief 5s bij `handed_over` (was 15s) via `threadStatus`-tracking, rest 15s — geen logica-wijzigingen.
- **UX 2026-09-15 — admin-inbox afwerking:** verzendknop icoon-only (pijltje, `h-11 w-11`, geen "Versturen"-tekst) + spinner-loading; verzenden zónder volledige thread-herlaad (optimistische bubbel + stille lijst/tellers-update, foto-URL achteraf gekoppeld); foto-lightbox ook in inbox (`#chatLightbox`, `z-[80]`, Escape). Live geverifieerd: pagina 200 + lightbox + icon-only knop.
- **Fix 2026-09-15 — admin-loader overviel foto-popup:** `loader.js` luistert in **capture**-fase en markeerde/toonde de fullscreen-loader (`z-index: 9999`) óók bij foto-kliks, vóórdat de lightbox-`preventDefault` (bubble) liep — resultaat: wazige pagina + "Laden..." over de popup. Nu slaat de loader `a[data-photo]` over. Les: capture-listeners winnen altijd van bubble-`preventDefault`.
- **Fix 2026-09-15 (deadline) — bot weigerde alles ("alleen Slimme-PC-vragen"):** oorzaak = embeddings-pad gaf [] terug bij lage sim → keyword-fallback werd NOOIT bereikt (vroege return) → korte queries zonder gronding + lange prompt → deflectie. Oplossingen: (1) **UNIE** van embeddings + keywords (sorteren op sim*10+score, geen blokkade) + threshold 0.30→0.25; (2) `ArabicText`-suffixen uitgebreid (ني/كم/هم/هن/ها/نا/ك/ه: تساعدني→تساعد, موقعكم→موقع); (3) expliciete begroetingsregel in prompt. **Meteen daarna 14/15, na tail-stripper 14/14**, cap-query 4–5/5 stabiel, unit-suites **34/34**.
- **Foto-flow 2026-09-14 (live getest):** paperclip → preview + naam + X → multipart `POST /ai-chat/send` (image jpg/png/webp/avif, 10MB) → `chat/{id}/{uuid}` op disk `local` → `photo_url` (token-guard, inline stream 200). Widget toont foto-bubbel + admin-inbox toont thumbnail klikbaar naar volledig. AI kan de foto NIET zien → `generate(..., $hasPhoto)`-nudge: eerlijk zeggen + vragen naar apparaat/probleem + vermelden dat medewerker hem wél ziet (geverifieerd AR-antwoord; handoff-knop verschijnt passend omdat mensenogen nodig zijn).

## 33. Update 2026-09-20 — Inbox responsive + ontvangst-camera + juridische CMS-pagina's + lidmaatschap

- **Chat/contact-inbox app-style responsive (design-only):** op mobiel (`<1024px`) verbergt `openThread()` de pagina-header (titel + badges) zodat de chat het scherm vult (zelfde patroon als contact-inbox al had: `isMobile()` via matchMedia + restore bij back/resize). Chat-header één regel (`AI aan`-tekst verborgen <sm, status-select `max-w-[6.5rem]`), meta-chips één horizontaal scrollbare rij i.p.v. 3 regels, composer-textarea `min-w-0` (was de oorzaak van het ingedrukte typveld — `.form-input{w-full}` + `flex-1` zonder `min-w-0`), bubbels `max-w-[92%]` + `break-words` op mobiel, footer verborgen op chat-route (lag alleen contact-inbox uitgesloten). Contact-inbox kreeg dezelfde behandeling + verzendknop icoon-only op mobiel. Bestanden: `admin/chat/inbox/index.blade.php`, `admin/contact-inbox/index.blade.php`, `chat-inbox.js`, `contact-inbox.js`, `components/admin/layout.blade.php` (footer-conditie + resize).
- **Fix — factuur-download bleef hangen:** `loader.js` intercepteert ELKE interne link-klik (fullscreen-loader); bij een download-response komt er geen navigatie → loader voor eeuwig. `Factuur`-knop in lidmaatschap-detail is nu een `<button>` (geen link) met fetch→blob→`a[download]` + `btnLoading`-spinner op de knop zelf. Les: download-acties nooit als plain `<a href>` zonder `download`-attribuut.
- **Ontvangst-foto's: bestanden/camera-keuze + live camera (`admin.bevestiging-mail.ontvangst.create`):** klik op dropzone/`Toevoegen`-tile opent `x-admin.modal`-popup (Bestanden vs Camera). Camera = live `getUserMedia`-view IN de popup (werkt op desktop-webcam én USB-camera via device-dropdown `enumerateDevices`, 1920×1080 ideal) + fallback native `capture="environment"`-input voor oude browsers. Capture → canvas → JPEG-File → bestaande `addFiles()`-pipeline (type/10MB/20-max, preview, counter). Stream stopt altijd bij sluiten (overlay/Annuleren/Escape/Terug). Let op: `capture`-attribuut wordt op desktop genegeerd (standaard browser-gedrag); USB-camera ja (video-device), flatbed-scanner nee (geen video-device → via Bestanden uploaden). `getUserMedia` vereist secure context (localhost ok, productie HTTPS).
- **Juridische CMS-pagina's (privacy + voorwaarden):** nieuwe `config/cms.php`-pages (elk één `inhoud`-sectie: badge/titel/intro/`content`-textarea/`updated_label`), sidebar-dropdown **Juridisch** (laatste item) met 2 sectie-edit-links, `PageController@privacy/@voorwaarden` (zelfde full-page-cache-patroon), routes `/privacy` + `/voorwaarden` (namen `privacy`/`voorwaarden`), views `landing/privacy|voorwaarden.blade.php` + shared partial `legal-page.blade.php` (design = site-stijl hero + kaart; `## `-prefix → `<h2>`, witregel → alinea, alles ge-escaped). Oude inhoud (8 privacy + 8 voorwaarden-rijen, platte tekst) gemigreerd via `LegalContentSeeder` (leest `database/seeders/data/legal-old.json`, `updateOrCreate` + `Cms::bust`). Footer-links `/voorwaarden` + `/privacy` waren al aanwezig maar 404 — werken nu; `service-reparatie` `/privacyverklaring` → `/privacy` geünificeerd; register-`href="#"` → echte routes (`target=_blank`); checkout kreeg verplichte terms-checkbox (`accepted`-rule + inline `data-err="terms"`).
- **Fix — register-breedte:** register-card was `390px`, login default `440px` → override verwijderd (beide 440px).
- **Lidmaatschap (`/lid-worden`, nieuw subsysteem, gespiegeld aan oud project `C:\laragon\www\slimmepc`):**
  - DB: `memberships` (klantnummer unique, type/gender/naam/contact, start/end, total, payment_status paid/unpaid/cancelled, mollie_payment_id, terms) + `membership_invoices` (nummer/subtotal/21%-btw/total/pdf_path) + `membership_settings` (één `subscription_price`) + gender-verbreding naar string(20).
  - Oud: `SubscriptionSetting` (1 prijs), `Membership` + `SubscriptionInvoice`, `lidword/lidstore` (validatie, dubbele-actieve-check, SMP-klantnummer, 1 jaar, factuur + `InvoiceService`-PDF-mail), Mollie prepare/return via sessie. Nieuw hergebruikt **`MolliePaymentService`** (was al voorbereid: `membership_id` in metadata-docblock) — niets opnieuw gebouwd.
  - Admin (`admin.lidmaatschap.*`, sidebar-dropdown **Lidmaatschap**): leden-tabel (zoeken/status-filter/paginatie/counts, zelfde inbox-design + `AdminTable.loading` + toast + `btnLoading`), detail-modal (chips + factuur-download + delete-modal), **Prijs-instelling** (singleton-prijs, min €1, `updateOrCreate`).
  - Frontend: `/lid-worden` exact uit `D:\sm 2026\slimmepc2026nieuwe\lidworden.html` in landing-layout (header-link `/lid-worden` bestond al dood in CMS-nav → werkt nu), **géén prijspublicatie** (prijs alleen server-side uit settings — nooit uit forminput), terms-links naar `/voorwaarden` + `/privacy`, AJAX-submit in afspraak-stijl (rood veld + melding eronder + focus; fout-`<p>` NA de `.relative`-wrapper geïnjecteerd zodat het icoon niet verschuift — les uit bug). `StoreMembershipRequest` (NL-regexen; telefoon internationaal versoepeld `/^\+?[0-9\s\-\/().]{6,25}$/`).
  - Flow: store → membership (unpaid, +1 jaar) + factuurrij (21%-splitsing) → Mollie via service → return/webhook (`lid-worden/webhook` CSRF-exempt in `bootstrap/app.php`) → `finalizeMembership` (idempotent: paid + PDF via `invoices/membership.blade.php` + `MembershipWelcomeMail` met factuur-PDF + looptijd, `afterResponse`) → success-pagina; mislukt → failed-pagina. Gasten mogen lid worden (geen login vereist; `user_id` nullable, klantnummer `SMP-XXXXX######`).
  - **Renewal-fix:** oude én nieuwe `unique`-email-rule blokkeerde verlenging van verlopen leden ("al geregistreerd"). Nu: alleen actief-betaalde lidmaatschappen blokkeren (controller-check, 422); verlopen/open/geannuleerd → nieuw jaar-rij (historie blijft). Live bewezen: verlopen → Mollie-redirect + nieuwe rij; actief → 422.
  - **Prijs-seed:** `MembershipPriceSeeder` (idempotent, alleen bij lege tabel, €23,00 = oude prijs), ook server-side gedraaid. Prijs wijzigen = dashboard (Lidmaatschap → Prijs-instelling).
- **CSS-les:** landing/admin gebruiken vooraf gecompilede Tailwind (`npm run css:landing` / `npm run css`) — nieuwe blade-klassen (zoals `max-w-[760px]`) bestaan pas na rebuild; `landing.css` + `app.css` zijn git-tracked dus deploy neemt ze mee. `filemtime`-cachebust regelt verversing.
- **Operationele les:** server is schone GitHub-mirror (`git reset --hard` wist hand-edits; untracked files blijven liggen!) — nooit handmatig bestanden naar de server kopiëren; alles via git + committed seeders + `scripts/deploy.sh`. Deploy.sh draait migrate maar géén seed → seeders eenmalig handmatig via ssh draaien.

## 34. Update 2026-09-22 — Monteur-betaalsysteem (technician payment flow)

- **Doel:** monteur logt in op locatie, kiest klant via klantnummer, vult werkformulier in, systeem berekent prijs (met lid- en couponkorting), klant betaalt via Mollie, factuur-PDF gaat per mail naar de klant. Gespiegeld aan oud project (`C:\laragon\www\slimmepc`: `TechnicianController`, `MollieController@prepare/handleTechnician*`, `TechnicianInvoiceService`, tabellen `technician_*`), maar met nieuwe Mollie-service, nieuw design en extra lidkortingen.
- **DB (zelfde als oud i.v.m. data-synchronisatie):** `technician_settings` (key/value), `technician_forms` (alle oude kolommen ongewijzigd + nieuw nullable: `member_discount`, `coupon_id`, `coupon_discount`, `mollie_payment_id`, `payment_method`), `technician_invoices` (identiek, `SLP-uniqid`-nummer via boot). `value`-kolom verruimd naar string(50) (integer accepteerde `'none'` niet; oude cijfers passen als string). Coupon-gebruik via nieuwe conventie (`CouponUsage` + `used_count`, GEEN `coupon_user`-pivot — die bestaat hier niet).
- **Klant aanmaken via register (géén aparte pagina):** `/register?nieuwe-klant` (zonder `=1`) — zelfde formulier, bij flag zijn telefoon/adres verplicht + hidden input. `User::boot()` genereert automatisch `SLP-######` (zoals oud: `SLP-` + 6 cijfers + uniciteits-loop) — OOK voor normale registraties. Admin-mail (`NewCustomerMail`, `CONTACT_NOTIFY_EMAIL`) met naam/e-mail/telefoon/**klantnummer** + CTA "Login als technicus". Géén auto-login bij flag (sessie van monteur mag niet wisselen — les uit bug: `guest`-middleware kaatste naar home); redirect naar `betaal/login` met groene status + klantnummer. Oude e-mailtekst ("Gebruik het klantnummer...") op verzoek verwijderd.
- **Monteur-login (`/betaal/login`, géén header/footer — `x-guest-layout` + `x-auth-card` 440px zoals login):** zelfde logica als oud (email + wachtwoord + klantnummer, `role=technician`, klant moet bestaan), nieuw design, placeholder `SLP-000000`, link "Nieuwe klant" naar register-flag. Success/failed-pagina's (`technician-success|failed`) eveneens zonder header/footer ("Nieuwe betaling"-knop op verzoek verwijderd).
- **Betaalpagina (`/technician/payment/{klantnummer}`, géén header/footer):** oude velden (start/eind, omschrijving, werkzaamheden, advies, rating 1–5 + comment zoals oud = alleen opslag, coupon) + **live prijsopbouw-kaart** (arbeid/kwartieren, reiskosten, lidkorting-badge, couponregel, **BTW (21%)**, totaal) via `/technician/quote` + `/technician/check-coupon` (AJAX). Validatie in afspraak-stijl: `reportValidity()` (native rode ballonnetjes) + 422 inline (rood veld + melding + focus). Prijs NOOIT uit formulier — altijd server-side.
- **Prijsformule (zoals oud, maar uurtarief i.p.v. kwartiertarief):** `ceil(minuten/15) × (uurprijs/4) + reiskosten` (incl.) → **lidkorting eerst** (percent of vast, + optioneel gratis reiskosten; check op klant-**e-mail** tegen actieve betaalde membership) → **daarna coupon** (shop-`Coupon`: `isActive()` + `discountAmount()` + per-user-check via `CouponUsage`) → 21% eruit (`×21/121`). Minimaal 5 minuten. Seeder-defaults = oud: €80/uur (€20/kwartier) + €5 reiskosten, lidkorting uit.
- **Betaling:** `MolliePaymentService` (alleen hergebruikt) + return/webhook (`technician/webhook` CSRF-exempt) + idempotente `finalizeForm` (paid + PDF `invoices/technician.blade.php` + `TechnicianInvoiceMail` met PDF naar klant, `afterResponse`).
- **Dashboard — Monteur-dropdown:** **Facturen** (zoeken/status/paginatie/counts, detail-modal, fetch-blob-download met knop-spinner zonder refresh — les uit lidmaatschap-fix, delete-modal; zelfde design-systeem + `monteur.js`) + **Tarieven** (uurprijs + reiskosten + kortingstype/waarde + gratis-reiskosten-toggle, eigen spinner i.p.v. globale `data-loading` — les: globale handler liet spinner eeuwig draaien bij AJAX). `TechnicianRatesSeeder` (idempotent) + server-side gedraaid.
- **Nummers (zoals oud):** klant = `SLP-######` (auto via `User::boot`), lidmaatschap = `SMP-XXXXX######`, facturen = `SLP-uniqid`, lid-facturen = `LID-######`.
- **Live bewezen lokaal:** login → betaalpagina (badge bij lid) → quote 45min = €65,00 (3×€20+€5) → submit → Mollie-redirect → factuurrij met juiste BTW-splitsing; lid (10% + gratis reis) 60min = €72,00; verlopen-lid verlengt, actief-lid 422. Testdata daarna opgeruimd.

## 35. Update 2026-09-26 — Monteur-betaalpagina afwerking (klantgegevens, PDF-logo, auto-logout)

- **Klantgegevens-kaart op betaalpagina (`landing/technician-payment.blade.php`):** onder de titel staat nu een `Klantgegevens`-blok met `Klantnummer:` / `Naam:` / `Adres:` (één regel `straat huisnummer, postcode stad`) / `Email:` / `Telefoon:` / `Datum:` (vandaag, `now()->format('d-m-Y')`) — label vet + waarde op dezelfde regel, elk item op eigen rij. De oude subtitel `{{ $client->name }} · {{ $client->klantnummer }}` onder de H1 is verwijderd (dubbel). Lege velden tonen `—`.
- **Rating-veld verwijderd:** `Beoordeling (1–5)`-select is uit het formulier gehaald (backend-`rating`-rule blijft `nullable`, dus oude submits breken niet). `Kortingscode` is daardoor `md:col-span-2` (volle breedte).
- **Veldvolgorde (definitief):** Omschrijving → Uitgevoerde werkzaamheden → Advies → **Starttijd/Eindtijd** (naast elkaar, direct vóór Kortingscode) → Kortingscode → Opmerking. Volgorde is pure presentatie (zelfde `id`/`name`, `FormData`-submit + server-validatie ongewijzigd).
- **Prijsopbouw nu instant in de browser (géén AJAX meer voor de basis-quote):** tarieven staan server-side in de pagina (`PRICING`-const: `quarter_price/travel_cost/member_free_travel/member_discount_type/member_discount_value/is_member` via simpele `{{ }}`-echo's — géén `@json([...])`, dat breekt de Blade-compiler met `Unclosed '[' does not match ')'`) en `refreshQuote()` rekent lokaal met exact dezelfde formule (`ceil(min/15)`, min. 5 min, lidkorting vóór coupon, BTW `×21/121`). Server blijft authoritatief bij opslaan (`calculate()` ongewijzigd). Debounce 200ms op `input`+`change`; fout (eind ≤ start) als Nederlandse hint-tekst onder Prijsopbouw.
- **Tweede onafhankelijk rekenpad (fallback):** `window.techQuoteFallback()` in een aparte `<script>`-tag (plain ES5, geen arrow/`?.`/template-literals) leest tarieven uit `data-*`-attributen op `#priceBox` en wordt direct via inline `oninput`/`onchange` op beide tijdvelden aangeroepen — als het hoofdscript ooit faalt, rekent dit pad alsnog. Beide paden zijn idempotent (zelfde paint).
- **CSP-les (belangrijk, 2026-09-26):** klant meldde dat Prijsopbouw nooit rekende; console toonde `script-src 'none'`-violations op inline + externe scripts. **Onze app stuurt géén CSP** (headers + HTML geverifieerd schoon) — de policy werd geïnjecteerd door de browser-omgeving van de klant (extensie type NoScript/antivirus-shield). Bewijs: betalen werkte wel (native form-POST, geen JS nodig), alleen de JS-quote stond stil. Diagnose-instructie: test in Incognito (`Ctrl+Shift+N`, extensies uit); werkt het daar → dader-extensie zoeken via `chrome://extensions` en `127.0.0.1` whitelisten. Les: bij "JS doet niets, geen eigen console-errors" altijd eerst naar CSP-violations kijken.
- **Monteur-PDF (`invoices/technician.blade.php`):** logo ontbrak (alleen tekst `Slimme-PC`) — nu hetzelfde base64-logo-blok als `order.blade.php` (CMS-logo → `logo.png`-candidates, webp→png-conversie voor dompdf, zwarte fallback-div). Subtitel `Monteursfactuur` onder het logo op verzoek verwijderd (alleen logo links).
- **Bedrijfsheader in ALLE 4 factuur-PDF's (`technician/order/hardware/membership`):** `.brand` (`Slimme-PC`) nu zwart `#000` (was blauw `#2563eb`) + groter `22px` (was `18px`), gewicht `800` behouden; `.header-right p` (adres/e-mail/tel/KVK/BTW/IBAN) nu zwart `#000` (was grijs `#475569`), normaal gewicht. Geldt alleen voor nieuwe PDF's (opgeslagen PDF's hergenereren niet vanzelf).
- **Auto-logout monteur na succes (`TechnicianController@success`):** bij tonen van `technician/success/{form}` (alleen bij `paid`, anders 404) wordt een ingelogde monteur direct uitgelogd (`Auth::logout()` + `session()->invalidate()` + `regenerateToken()`, zelfde als Breeze-logout) — veiligheid op locatie/apparaat van de klant. `failed`-pagina logt NIET uit (opnieuw proberen zonder herlogin).
- **Prijsbox-layout (2026-09-26, exact op verzoek):** `Aantaaluuren: {h}uur {m}min` (duur, géén los Arbeid-bedrag meer) → `Reiskost: € X,XX` (altijd bedrag) → witregel → `Subtotaal (excl. btw):` → witregel → `Btw (21%):` → witregel → `Totaal (incl. btw):` (vet/blauw). Euro-notatie in deze box mét spatie (`€ 9,00`). Lid/kortingscode-rijen alleen zichtbaar bij korting > 0. `paintQuote` + fallback herschreven op `prAantal/prSub`-ids; coupon-handler ververst nu ook `prSub` (server levert `subtotal` in beide endpoints).
- **Opmerking-veld verborgen (frontend-only):** `comment`-textarea uit `technician-payment.blade.php` gehaald; backend-`comment`-rule blijft `nullable` (oude data/submits breken niet).
- **Factuur-header restyle (2026-09-26, alle 4 PDF's):** logo `120px` → `90px`; bedrijfsnaam `Slimme-PC` → `20px` blauw `#2563eb` (vet 800; `30px` te groot, `24px` nog te groot bevonden); adres-regels met vette labels (`Adres:` = `Asselsestraat 24, 7311EL Apeldoorn`, `Email/Tel/KVK/BTW/IBAN`, waarden ongewijzigd); `Monteursfactuur`-subtitel weg bij technician; membership heeft géén logo (alleen tekst-header). **Specificity-les:** `.header-right p` (9px, spec. 0-1-1) won altijd van `.brand` (0-1-0) waardoor de merknaam klein/zwart bleef — opgelost met expliciete `.header-right p.brand`-override (0-2-1) in alle 4 templates.
- **Labels betaalpagina (2026-09-26, op verzoek):** H1 `Betaling klant` → `Betaling formulier`; klantblok zonder `Klantgegevens`-titel, label + waarde op één regel (`Klantnummer/Naam/Adres/Email/Telefoon/Datum`); `Omschrijving` → `Probleem omschrijven`, `Uitgevoerde werkzaamheden` → `Werkzaamheden`, `Starttijd *` → `Van *`, `Eindtijd *` → `Tot *`, `Prijsopbouw` → `Aantaaluuren` (exacte spelling op verzoek), `Kortingscode` → `Heb je een kortingscode?`, `Voorrijkosten` → `Reiskost` (bedrag altijd getoond).
