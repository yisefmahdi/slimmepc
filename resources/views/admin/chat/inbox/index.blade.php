<x-admin.layout title="Live Chat Inbox">
    ...
    <div class="inbox-app flex h-[calc(100dvh-108px)] min-h-[24rem] flex-col overflow-hidden lg:h-[calc(100dvh-9rem)] lg:min-h-[26rem]">
        ...
        <div class="inbox-panes grid min-h-0 flex-1 grid-cols-1 grid-rows-[minmax(0,1fr)] gap-4 overflow-hidden lg:grid-cols-[minmax(0,19rem)_minmax(0,1fr)]">
            ...
            <div id="chatListPane"
                ...
                <div class="shrink-0 border-b px-3 py-2.5 sm:px-4 sm:py-3" style="border-color: rgba(148, 163, 184, 0.15)">
                    <div class="relative">
                        ...
                        <input type="text" id="chatSearch" placeholder="Zoek op naam, e-mail of bericht..."
                    ...
                    <div class="mt-2.5 flex items-center gap-2">
                        <select id="chatStatusFilter"
                            ...
                            <option value="handed_over">Medewerker</option>
                            <option value="closed">Gesloten</option>
                            <option value="offline">Offline</option>
                        </select>
                        <select id="chatPerPage"
                ...
                {{-- List (scrolls) --}}
                <div id="chatList" class="inbox-scroll min-h-0 flex-1 overflow-y-auto px-2 py-2">
                    <div class="flex flex-col items-center gap-3 px-6 py-16 text-center">
                        ...
                        <p class="font-semibold" style="color: var(--c-heading)">Geen gesprekken gevonden</p>
                        <p class="text-xs" style="color: var(--c-muted)">Pas je zoekopdracht of filters aan.</p>
                    </div>
                </div>

                {{-- Pagination (compact) --}}
                <div id="chatPagination" class="inbox-pagination shrink-0 border-t px-3 py-2" style="border-color: rgba(148, 163, 184, 0.15)"></div>
            </div>
            ...
            <div id="chatPane"
                ...
                {{-- Empty state --}}
                <div id="chatEmpty" class="flex h-full flex-col items-center justify-center gap-3 px-6 py-10 text-center">
                    ...
                    <p class="font-semibold" style="color: var(--c-heading)">Selecteer een gesprek</p>
                    <p class="max-w-xs text-xs" style="color: var(--c-muted)">Klik links op een gesprek om klant- en AI-berichten te zien en te antwoorden (AI stopt dan).</p>
                </div>
                ...
                <div id="chatThread" class="hidden min-h-0 min-w-0 flex-1 flex-col">
                    ...
                    <div class="shrink-0 border-b px-3 py-2.5 sm:px-5 sm:py-3.5" style="border-color: rgba(148, 163, 184, 0.15)">
                        <div class="flex items-center justify-between gap-2 sm:gap-3">
                            ...
                            <div class="flex shrink-0 items-center gap-1.5 sm:gap-2">
                                ...
                                <button type="button" id="chatDeleteBtn" title="Verwijder"
                    ...
                    <div class="shrink-0 border-t px-3 py-3 sm:px-5 sm:py-4" style="border-color: rgba(148, 163, 184, 0.15)">
                        ...
                        <div id="chatComposer">
                        <p class="mb-2 hidden text-[11px] font-bold text-amber-600 dark:text-amber-400" id="chatAiNote">AI is aan — jouw antwoord schakelt de AI uit voor dit gesprek.</p>
                        <div id="chatAttachChip" class="mb-3 hidden items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold"
                             style="border-color: rgba(148, 163, 184, 0.25); color: var(--c-heading)">
                            <span id="chatAttachName" class="truncate"></span>
                            <button type="button" id="chatAttachRemove" title="Foto verwijderen"
                        ...
                        <div class="flex min-w-0 items-end gap-2 sm:gap-2.5">
                            ...
                            <button type="button" id="chatReplyBtn" aria-label="Versturen" title="Versturen"
                                    class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-[#075be8] to-[#064bd7] text-white shadow-[0_10px_25px_rgba(0,91,234,0.25)] transition duration-300 hover:-translate-y-0.5 disabled:opacity-60">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="h-5 w-5">
    ...
    {{-- ============ Foto lightbox (popup) ============ --}}
    <div id="chatLightbox" class="fixed inset-0 z-[80] hidden flex-col items-center justify-center bg-slate-950/85 p-4 backdrop-blur-sm" role="dialog" aria-label="Foto vergroten" aria-hidden="true">
        <div class="relative rounded-2xl bg-white p-2 shadow-2xl" style="max-width:min(380px,88vw);">
            <button type="button" id="chatLightboxClose" aria-label="Sluiten"
            ...
            <img id="chatLightboxImg" src="" alt="Foto vergroot" class="w-auto max-w-full rounded-xl object-contain" style="max-height:70vh;">
        </div>
    </div>

    {{-- ============ Delete confirm modal ============ --}}
    <x-admin.modal id="chatDeleteModal" title="Gesprek verwijderen" size="sm">
        <div class="flex items-start gap-4">
            ...
            <div>
                <p class="text-sm font-semibold" style="color: var(--c-heading)">Weet je zeker dat je
                    <span id="chatDeleteName" class="font-bold">dit gesprek</span> wilt verwijderen?</p>
                <p class="mt-1 text-xs leading-5" style="color: var(--c-muted)">
                    Alle berichten, foto's en de beoordeling worden permanent verwijderd.
                </p>
            </div>
        </div>
        ...
        <x-slot name="footer">
            ...
            <button type="button" id="chatDeleteConfirmBtn"
    ...
    <script src="{{ asset('assets/js/admin/chat-inbox.js') }}?v={{ filemtime(public_path('assets/js/admin/chat-inbox.js')) }}"></script>
</x-admin.layout>
