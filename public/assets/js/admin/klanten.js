...
/* ---------- data loading ---------- */
async function load() {
    if (window.AdminTable) window.AdminTable.loading(document.getElementById('klantTableBody'), 6);
    try {
...
async function confirmDelete() {
    ...
    const deletedId = state.deletingId;
    try {
        await axios.delete(`/admin/users/${deletedId}`);
        ...
        if (row) row.remove();
        if (!document.querySelectorAll('#klantTableBody tr').length) {
            document.getElementById('klantTableBody').innerHTML = `<tr><td colspan="6" class="px-6 py-14 text-center"><div class="flex flex-col items-center gap-3"><span class="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-blue-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-7 w-7"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0Zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0Z" /></svg></span><p class="font-semibold" style="color: var(--c-heading)">Geen klanten gevonden</p><p class="text-xs" style="color: var(--c-muted)">Pas je zoekopdracht of filters aan, of voeg een nieuwe klant toe.</p></div></td></tr>`;
        }
